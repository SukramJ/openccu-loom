...
var valueSemanticsChanges = []string{
	...
	"10.0.0 (wsapi) links.test_profile -> links.apply_profile: the command was removed and replaced rather than renamed, because its behaviour changed as well. test_profile returned a profile's values under the field name applied_values and wrote nothing; apply_profile writes them to the LINK paramset and requires the per-link edit token. The name follows the firmware, which keeps writing (cmd_set_profile) and triggering (cmd_activateLinkParamset) apart and which this daemon already mirrors as links.activate_paramset; the CCU-side reference folds the write under the word test, and that divergence is recorded in notes/parity/by_design.md. The written set is the profile's fixed values plus the declared defaults of its loose constraints — a loose constraint with no default is left at the device's current value, so applying a profile does not guarantee that a later links.get_profiles reports it as active",
	...
	"10.3.0 SimpleScheduleEntry.duration / .ramp_time: vocabulary — the reserved word \"permanent\" joined the value set. Both fields are strings either way, so no schema diff sees it. It is the CCU's own encoding for a switch point that does not expire (DURATION_BASE 7, DURATION_FACTOR 31 — the pair its weekly-program editor writes for the first of its two options, labelled \"Dauerhaft\"), and until now this daemon decoded that pair to the empty string. A client therefore could not tell a slot the device holds forever from one carrying no duration at all, and had no way to set the former. The empty string keeps its own meaning — leave the device's duration untouched — so the two are distinct now rather than merged",
	"10.4.0 StartupCaptureConfig / ScheduleWriteResult / ScheduleTimeCorrection: meaning — all three stopped declaring additionalProperties: false. No property moved, so no schema diff sees it; what changed is how a generated client decodes them. The keyword became extra=\"forbid\" in openccu-loom-client's pydantic models, which made adding a field to any of the three a break for every installed client while every diff here called it additive. Strictness stays on the request side (StartupCaptureConfigWrite), where the daemon is the decoder and an unknown key is a caller's typo worth naming. A client generated before this still rejects unknown properties on the three, so the tolerance arrives with its next regeneration, not retroactively",
	...
	"11.0.0 DownloadSystemFirmwareRequest.url: meaning — the field is accepted and ignored, and it stopped being required. The endpoint offered to fetch a caller-supplied image onto the CCU; no CCU call takes one. Its own entry point resolves the download from the box's version and board serial, so the URL had nowhere to go: the previous implementation POSTed an action the maintenance CGI does not define and read the resulting HTML error page, served under HTTP 200, as success — which is why every call answered 202 whatever the URL said. The field stays in the schema so a client written against the old contract keeps working rather than failing validation, and it is marked deprecated. What genuinely changes for that client is the answer: a 202 now means the CCU reported the download succeeded, and a 502 means it reported a failure. No property moved, so no schema diff sees any of it",
	"11.1.0 config import (POST /devices/{addr}/channels/{no}/paramsets/{key}/config/import) and WS paramset.copy: meaning — a MASTER or LINK snapshot now passes the same per-resource edit lock as PUT /devices/{addr}/paramsets/{key} and is refused 423 without an X-Edit-Token; the request shape is unchanged, so no diff sees it. Alongside: oidcCallback answers 303 instead of 302 (same redirect, method-safe), PUT /devices/{addr}/paramsets/LINK is refused 400 before any CCU call (the LINK paramset is addressed per peer), the anonymous rate limit no longer covers /app/* and /health, three lenient handlers cap bodies at 1 MiB, and the WS subscribe ACK is unconditional",
}
