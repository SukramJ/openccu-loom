...
var wiringFuncsWithoutSeam = map[string]string{
	"registerFirmwareJobs":              "constructs a value and returns it; the caller decides where it goes, so there is no handover here to declare",
	...
	"wireConfigStoreCrypto":             "belongs to hmcli, which builds no central registry and therefore has no manifest",
	...
	"wireHubDiscoveryOnReady":           "starts collaborators that each declare their own seam through OnRegisterDeclared, so the entries exist one level down",
