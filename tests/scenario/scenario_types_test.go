...
type scenarioGiven struct {
	...
	// scenario_topologies_test.go that assembles a fleet with
	// measurement sources / cluster servers. Default (empty) uses the
	// empty topology — root plus aggregator, no bridged endpoint.
	// Recipes are needed by scenarios that exercise the
	// notifier-wiring path or read real cluster-server output
...
// the path set are supported: a single subscription object or paths
// as an array — multi-path scenarios use the array.
type scenarioSubSpec struct {
	...
	// SkipAutoSubscribe leaves the subscription manager empty during
	// setup so a step's own peer.send_subscribe_request drives the
	// registration through the production handleSubscribeRequest
	// path. Defaults to false: the harness then establishes the
	// subscription itself, also over the wire, before the first step
	// runs.
	SkipAutoSubscribe bool `json:"skip_auto_subscribe,omitempty"`

	// MinIntervalFloorSeconds + MaxIntervalCeilingSeconds override the
	// harness default (1 s / 60 s) in the setup SubscribeRequest. Used
	// by cadence scenarios to test the MinInterval gate against a
	// longer floor without slowing the rest of the suite. Zero means
	...
	// does NOT tick on its own. Cadence scenarios then drive the
	// engine via engine_tick_at with controlled wall-clock offsets,
	// making the MinInterval-gate and MaxInterval-keepalive paths
	// deterministic without wall-clock waits. NOT compatible with
	// fire_via_engine — those steps rely on the engine goroutine to
	// drain dirty marks.
	EngineManualOnly bool `json:"engine_manual_only,omitempty"`

	// FabricIndex maps the per-subscription session to a fabric so
	// the bridge's session-fabric resolver chain stamps a non-zero
	// FabricIndex into the dispatch context. Defaults to 0
	// (pre-fabric / PASE), which is what the production daemon
	// resolves for an unresolved session. Fabric-scoped scenarios set
	// this so fabric-scoped cluster servers see a meaningful fabric
	// ID.
...
type scenarioStep struct {
	...
	// scenarioGiven.Subscriptions / the single-subscription slot the
	// step targets. Default 0 (the primary subscription).
	...
	// by narrowing scenarios that verify only the notifier's own
	...
	// Apple-write fields. Path defaults to the active subscription's
	// primary path when empty. PeerExchangeID is the exchange the
	// peer opens for the WriteRequest (the bridge's WriteResponse
	// echoes it back). Defaults to a deterministic per-scenario value
	// when zero.
	...
	// FabricFiltered flag (Matter §10.6.3). Used by fabric-scoped
	// read scenarios. Defaults to false.
	...
	// BindDataVersionTo binds the DataVersion uint32 from the first
	// AttributeDataIB into a $var, so assert_gt can compare it across
	// two reports.
	BindDataVersionTo string           `json:"bind_data_version_to,omitempty"`
	GT                *scenarioCompare `json:"gt,omitempty"`

	// BindAttributeValueTo extracts the first AttributeDataIB's Data
	// field as a uint64 and binds it. Used by fabric-scoped read
	// scenarios to verify the value the bridge encoded matches the
	// FabricIndex injected via the session resolver.
	// ExpectAttributeValue asserts equality with a literal.
	...
	// BindMaxIntervalTo extracts MaxInterval (context tag 2 uint16)
	// from a SubscribeResponseMessage payload and binds it.
	...
	// AtMillis is the offset in milliseconds from the harness's
	// setup-time anchor t0. engine_tick_at calls
...
func loadScenarioFile(path string) (*scenario, error) {
	raw, err := os.ReadFile(path) //nolint:gosec // repo-anchored corpus path.
	if err != nil {
