...
func TestParityMatterJS_EndpointNumberNotReissuedAfterRemoval(t *testing.T) {
	...
	keyA := testKey("ccu1", "AAA0001", 1, matterendpoint.DPKindCustom, "SWITCH")
	keyB := testKey("ccu1", "BBB0002", 1, matterendpoint.DPKindCustom, "SWITCH")
	keyC := testKey("ccu1", "CCC0003", 1, matterendpoint.DPKindCustom, "SWITCH")

	idA, err := s.UpsertEndpointAssigning(ctx, fabricendpoint.Record{Key: keyA})
	if err != nil {
		t.Fatalf("assign A: %v", err)
	}
	idB, err := s.UpsertEndpointAssigning(ctx, fabricendpoint.Record{Key: keyB})
	if err != nil {
	...
	idC, err := s.UpsertEndpointAssigning(ctx, fabricendpoint.Record{Key: keyC})
	if err != nil {
...
func TestParityMatterJS_EndpointNumberFreshAfterReAdd(t *testing.T) {
	...
	key := testKey("ccu1", "DDD0004", 1, matterendpoint.DPKindCustom, "SWITCH")

	first, err := s.UpsertEndpointAssigning(ctx, fabricendpoint.Record{Key: key})
	if err != nil {
	...
	second, err := s.UpsertEndpointAssigning(ctx, fabricendpoint.Record{Key: key})
	if err != nil {
...
func TestParityMatterJS_EndpointNumberSeedsAboveExistingRows(t *testing.T) {
	...
	for i, id := range []uint16{2, 3, 5} {
		key := testKey("ccu1", "EEE0005", i+1, matterendpoint.DPKindCustom, "SWITCH")
