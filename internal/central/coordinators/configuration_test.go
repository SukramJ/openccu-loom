...
func TestConfigurationCoordinatorGetParameterData(t *testing.T) {
	...
	pd, ok := c.GetParameterData(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyMaster, "TRANSMIT_TRY_MAX")
	if !ok || pd.Type != hmenum.ParameterTypeInteger {
		t.Fatalf("hit miss: %+v ok=%v", pd, ok)
	}

	if _, ok := c.GetParameterData(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyMaster, "MISSING"); ok {
		t.Fatal("missing param must report ok=false")
	}
	if _, ok := c.GetParameterData(hmenum.InterfaceHmIPRF, "OTHER:1", hmenum.ParamsetKeyMaster, "TRANSMIT_TRY_MAX"); ok {
...
func TestConfigurationCoordinatorGetChannelParamset(t *testing.T) {
	c, pss, _ := newCoord(t)
	pss.Put(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyValues, hmproto.Paramset{
		"LEVEL":   hmproto.ParameterData{Type: hmenum.ParameterTypeFloat},
		"WORKING": hmproto.ParameterData{Type: hmenum.ParameterTypeBool},
	})

	got, ok := c.GetChannelParamset(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyValues)
	if !ok || len(got) != 2 {
	...
	again, _ := c.GetChannelParamset(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyValues)
...
func TestConfigurationCoordinatorPatchOverridesRegistry(t *testing.T) {
	...
	full, _ := c.GetChannelParamset(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyMaster)
	...
	pd, _ = c.GetParameterData(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyMaster, "TRANSMIT_TRY_MAX")
	if string(pd.Min) != "0" {
...
func TestConfigurationCoordinatorHasParameter(t *testing.T) {
	...
	if !c.HasParameter(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyValues, "LEVEL") {
		t.Fatal("LEVEL must be reported as present")
	}
	if c.HasParameter(hmenum.InterfaceHmIPRF, "0001ABCD:1", hmenum.ParamsetKeyValues, "GHOST") {
...
func TestConfigurationCoordinatorConfigurableChannels(t *testing.T) {
	...
	got := c.ConfigurableChannels(hmenum.InterfaceHmIPRF)
	if len(got) != 1 {
