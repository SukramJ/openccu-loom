...
// TestPutParamsetRefusesTheLiteralLinkKey pins that a paramset write
// keyed "LINK" never reaches the wire.
//
// The CCU addresses a LINK paramset by the *peer channel address*, not
// by a key named LINK — that literal is only a getParamsetDescription
// key. Sending it as putParamset's paramset argument is not a clean
// fault everywhere: on a BidCos channel that does have a LINK set, the
// firmware parses "LINK" as a peer address, fails, ignores the failure
// and commits a config write for peer 0xFFFFFF:0xFF over the air. The
// per-peer route [ParamsetsDomain.PutLinkParamset] is the addressable
// one; this route refuses instead, as the MCP tool already does.
func TestPutParamsetRefusesTheLiteralLinkKey(t *testing.T) {
	t.Parallel()

	domain, chw, fakeOps := buildParamsetFixture(t)
	putCalled := false
	fakeOps.putParamsetFn = func(_ context.Context, _ string, _ hmenum.ParamsetKey, _ map[string]any) error {
		putCalled = true
		return nil
	}
	...
	if putCalled {
		t.Error("a LINK-keyed write still reached the backend")
	}
