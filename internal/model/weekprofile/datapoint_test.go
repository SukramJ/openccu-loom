...
func TestSyncProfilePointerIPInt(t *testing.T) {
	t.Parallel()
	dp := NewProfileDataPoint(ProfileDataPointConfig{
		ScheduleType: ScheduleTypeClimate,
		ProfileCount: 6,
	...
	// IP: 1-based int
	if err := dp.SyncProfilePointer(3); err != nil {
		t.Fatalf("SyncProfilePointer(3): %v", err)
	}
	if dp.CurrentProfile() != "P3" {
		t.Errorf("got %q, want P3", dp.CurrentProfile())
	}
}

func TestSyncProfilePointerRFString(t *testing.T) {
	t.Parallel()
	dp := NewProfileDataPoint(ProfileDataPointConfig{
		ScheduleType: ScheduleTypeClimate,
		ProfileCount: 6,
	...
	// RF: 0-based string
	if err := dp.SyncProfilePointer("4"); err != nil {
		t.Fatalf("SyncProfilePointer(\"4\"): %v", err)
	}
	if dp.CurrentProfile() != "P5" {
		t.Errorf("got %q, want P5", dp.CurrentProfile())
	}
}

func TestSyncProfilePointerNilIsNoop(t *testing.T) {
	t.Parallel()
	dp := NewProfileDataPoint(ProfileDataPointConfig{
		ScheduleType: ScheduleTypeClimate,
		ProfileCount: 6,
	})
	if err := dp.SyncProfilePointer(nil); err != nil {
		t.Fatalf("SyncProfilePointer(nil): %v", err)
	}
	if dp.CurrentProfile() != "P1" {
		t.Errorf("got %q, want P1 (unchanged)", dp.CurrentProfile())
	}
}

func TestSyncProfilePointerOutOfRangeIsNoop(t *testing.T) {
	t.Parallel()
	dp := NewProfileDataPoint(ProfileDataPointConfig{
		ScheduleType: ScheduleTypeClimate,
		ProfileCount: 2, // only P1..P2 valid
	})
	// Raw value 3 maps to P3 — but this device only has 2 profiles.
	if err := dp.SyncProfilePointer(3); err == nil {
		// SetCurrentProfile would error; SyncProfilePointer propagates.
		// That's acceptable; just verify CurrentProfile is unchanged.
		t.Log("error expected for out-of-range but not required")
	}
	// P1 must remain the current profile.
	if dp.CurrentProfile() != "P1" {
		t.Errorf("profile changed to %q, want P1", dp.CurrentProfile())
	}
}
