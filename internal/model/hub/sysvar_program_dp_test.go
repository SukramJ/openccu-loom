...
// TestSysvarDpNumberSendVariableInRange verifies that a value within
// [min, max] is forwarded to the writer.
func TestSysvarDpNumberSendVariableInRange(t *testing.T) {
	w := &stubSysvar{}
	minVal := hmtypes.FloatValue(0.0)
	maxVal := hmtypes.FloatValue(100.0)
	sv := NewSysvar("c1", "temp", "", hmenum.HubValueTypeFloat, w)
	sv.Min = &minVal
	sv.Max = &maxVal
	num := &SysvarDpNumber{Sysvar: sv}

	if err := num.SendVariable(context.Background(), 50.0); err != nil {
		t.Fatalf("SendVariable(50.0) unexpected error: %v", err)
	}
	got, ok := num.ConfirmedValue()
	// Confirmed value is NOT set (writer succeeded, CCU has not replied yet).
	// But Value() returns the unconfirmed value.
	v, _ := num.Value()
	_ = ok
	_ = got
	if v.Kind != hmtypes.ValueKindFloat || v.Float != 50.0 {
		t.Errorf("unconfirmed Value()=%+v, want Float(50.0)", v)
	}
}

// TestSysvarDpNumberSendVariableBelowMin verifies that a value below min
// is rejected with an error.
func TestSysvarDpNumberSendVariableBelowMin(t *testing.T) {
	w := &stubSysvar{}
	minVal := hmtypes.FloatValue(10.0)
	maxVal := hmtypes.FloatValue(50.0)
	sv := NewSysvar("c1", "temp", "", hmenum.HubValueTypeFloat, w)
	sv.Min = &minVal
	sv.Max = &maxVal
	num := &SysvarDpNumber{Sysvar: sv}

	if err := num.SendVariable(context.Background(), 5.0); err == nil {
		t.Fatal("SendVariable(5.0) must error when min=10")
	}
}

// TestSysvarDpNumberSendVariableAboveMax verifies that a value above max
// is rejected with an error.
func TestSysvarDpNumberSendVariableAboveMax(t *testing.T) {
	w := &stubSysvar{}
	minVal := hmtypes.FloatValue(0.0)
	maxVal := hmtypes.FloatValue(30.0)
	sv := NewSysvar("c1", "bright", "", hmenum.HubValueTypeFloat, w)
	sv.Min = &minVal
	sv.Max = &maxVal
	num := &SysvarDpNumber{Sysvar: sv}

	if err := num.SendVariable(context.Background(), 31.0); err == nil {
		t.Fatal("SendVariable(31.0) must error when max=30")
	}
}

// TestSysvarDpNumberSendVariableNoBounds verifies that when Min or Max
// is nil the bounds check is skipped and any value is accepted.
func TestSysvarDpNumberSendVariableNoBounds(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "x", "", hmenum.HubValueTypeFloat, w)
	// no Min/Max
	num := &SysvarDpNumber{Sysvar: sv}

	if err := num.SendVariable(context.Background(), -9999.0); err != nil {
		t.Fatalf("SendVariable without bounds unexpected error: %v", err)
	}
}

// TestSysvarDpNumberIntMinMax verifies that integer-kind min/max bounds
// work correctly.
func TestSysvarDpNumberIntMinMax(t *testing.T) {
	w := &stubSysvar{}
	minVal := hmtypes.IntValue(1)
	maxVal := hmtypes.IntValue(5)
	sv := NewSysvar("c1", "count", "", hmenum.HubValueTypeInteger, w)
	sv.Min = &minVal
	sv.Max = &maxVal
	num := &SysvarDpNumber{Sysvar: sv}

	if err := num.SendVariable(context.Background(), 3.0); err != nil {
		t.Fatalf("in-range: %v", err)
	}
	if err := num.SendVariable(context.Background(), 6.0); err == nil {
...
// TestSysvarDpSelectValueFromIndex verifies that SelectValue returns the
// string label for the current numeric index.
func TestSysvarDpSelectValueFromIndex(t *testing.T) {
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, nil)
	sv.ValueList = []string{"off", "on", "eco"}
	sel := &SysvarDpSelect{Sysvar: sv}

	sv.OnValue(hmtypes.IntValue(1)) // "on"
	got, ok := sel.SelectValue()
	if !ok || got != "on" {
		t.Fatalf("SelectValue()=%q ok=%v, want %q true", got, ok, "on")
	}
}

// TestSysvarDpSelectValueOutOfRange returns false for an out-of-range index.
func TestSysvarDpSelectValueOutOfRange(t *testing.T) {
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, nil)
	sv.ValueList = []string{"a", "b"}
	sel := &SysvarDpSelect{Sysvar: sv}
	sv.OnValue(hmtypes.IntValue(99))
	_, ok := sel.SelectValue()
	if ok {
		t.Fatal("SelectValue() must return false for out-of-range index")
	}
}

// TestSysvarDpSelectSendVariableByIndex verifies that sending an integer
// index writes the correct value.
func TestSysvarDpSelectSendVariableByIndex(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, w)
	sv.ValueList = []string{"off", "on", "eco"}
	sel := &SysvarDpSelect{Sysvar: sv}

	if err := sel.SendVariable(context.Background(), 2); err != nil {
		t.Fatalf("SendVariable(2) unexpected error: %v", err)
	}
	...
	if pair[0] != "mode" {
		t.Errorf("writer name=%v", pair[0])
	}
	if pair[1].(int) != 2 {
		t.Errorf("writer value=%v, want 2", pair[1])
	}
}

// TestSysvarDpSelectSendVariableByString verifies that sending a string
// label resolves to the correct index.
func TestSysvarDpSelectSendVariableByString(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, w)
	sv.ValueList = []string{"off", "on", "eco"}
	sel := &SysvarDpSelect{Sysvar: sv}

	if err := sel.SendVariable(context.Background(), "eco"); err != nil {
		t.Fatalf("SendVariable(eco) unexpected error: %v", err)
	}
	pair := w.last.Load().([2]any)
	if pair[1].(int) != 2 {
		t.Errorf("expected index 2 for 'eco', got %v", pair[1])
	}
}

// TestSysvarDpSelectSendVariableUnknownString rejects unknown labels.
func TestSysvarDpSelectSendVariableUnknownString(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, w)
	sv.ValueList = []string{"a", "b"}
	sel := &SysvarDpSelect{Sysvar: sv}
	if err := sel.SendVariable(context.Background(), "zzz"); err == nil {
		t.Fatal("unknown string label must return error")
	}
}

// TestSysvarDpSelectSendVariableOutOfRangeIndex rejects out-of-range index.
func TestSysvarDpSelectSendVariableOutOfRangeIndex(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, w)
	sv.ValueList = []string{"a", "b"}
	sel := &SysvarDpSelect{Sysvar: sv}
	if err := sel.SendVariable(context.Background(), 5); err == nil {
		t.Fatal("out-of-range index must return error")
	}
}

// TestSysvarDpSelectNoValueList rejects writes when no value list is set.
func TestSysvarDpSelectNoValueList(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, w)
	sel := &SysvarDpSelect{Sysvar: sv}
	if err := sel.SendVariable(context.Background(), 0); err == nil {
		t.Fatal("empty value list must return error")
	}
}
...
// TestWrapSysvarReturnsNumber verifies that a writable float sysvar is
// wrapped as SysvarDpNumber.
func TestWrapSysvarReturnsNumber(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "n", "", hmenum.HubValueTypeFloat, w)
	got := WrapSysvar(sv)
	if _, ok := got.(*SysvarDpNumber); !ok {
		t.Fatalf("WrapSysvar(float+writer) = %T, want *SysvarDpNumber", got)
	}
}

// TestWrapSysvarReturnsNumberForInteger verifies that a writable integer
// sysvar is also wrapped as SysvarDpNumber.
func TestWrapSysvarReturnsNumberForInteger(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "n", "", hmenum.HubValueTypeInteger, w)
	got := WrapSysvar(sv)
	if _, ok := got.(*SysvarDpNumber); !ok {
		t.Fatalf("WrapSysvar(integer+writer) = %T, want *SysvarDpNumber", got)
	}
}

// TestWrapSysvarReturnsSelect verifies that a writable list sysvar is
// wrapped as SysvarDpSelect.
func TestWrapSysvarReturnsSelect(t *testing.T) {
	w := &stubSysvar{}
	sv := NewSysvar("c1", "mode", "", hmenum.HubValueTypeList, w)
	got := WrapSysvar(sv)
	if _, ok := got.(*SysvarDpSelect); !ok {
		t.Fatalf("WrapSysvar(list+writer) = %T, want *SysvarDpSelect", got)
	}
}

// TestWrapSysvarReturnsBaseForReadOnlyNumber verifies that a read-only
// numeric sysvar is returned as plain *Sysvar (no write wrapper).
func TestWrapSysvarReturnsBaseForReadOnlyNumber(t *testing.T) {
	sv := NewSysvar("c1", "n", "", hmenum.HubValueTypeFloat, nil)
	got := WrapSysvar(sv)
	if _, ok := got.(*Sysvar); !ok {
		t.Fatalf("WrapSysvar(float,no writer) = %T, want *Sysvar", got)
	}
}
