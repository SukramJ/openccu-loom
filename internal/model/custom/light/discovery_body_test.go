...
// haStrings reads a string list out of a flattened body.
//
// A JSON array decodes to []any, not []string — the same reason haNum exists.
// Asserting on the Go slice type would be asserting on how the body happened
// to be built rather than on what Home Assistant receives.
func haStrings(t *testing.T, body map[string]any, key string) []string {
	t.Helper()
	raw, ok := body[key]
	if !ok {
		t.Fatalf("body has no %q", key)
	...
	list, ok := raw.([]any)
	if !ok {
		t.Fatalf("body[%q] = %v (%T), want a list", key, raw, raw)
	}
	out := make([]string, 0, len(list))
	for _, v := range list {
		s, ok := v.(string)
		if !ok {
			t.Fatalf("body[%q] contains %v (%T), want strings", key, v, v)
		}
		out = append(out, s)
	}
	return out
