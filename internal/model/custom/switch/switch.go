...
type Switch struct {
	*generic.Switch
	...
	// non-nil they appear as additional cluster servers on the
	// Switch's bridged Matter endpoint (ElectricalPowerMeasurement
	// 0x0090 + ElectricalEnergyMeasurement 0x0091). nil for switches
	// that have no energy hardware (the default HmIP-FSM3 / etc.).
	attachedPowerSource  interfaces.MatterFloatMeasurementSource
	attachedEnergySource interfaces.MatterFloatMeasurementSource
