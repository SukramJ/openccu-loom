	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(pr chi.Router) {
			...
			if d.Alarm != nil {
				...
				pr.With(op).Post("/alarm/zones/{id}/reset-motion", handlers.ResetAlarmZoneMotion(d.Alarm, d.AuditRecorder))
				pr.With(op).Post("/alarm/reset-motion", handlers.ResetAllAlarmMotion(d.Alarm, d.AuditRecorder))
				pr.Get("/alarm/triggered-motion", handlers.ListAlarmTriggeredMotion(d.Alarm))
