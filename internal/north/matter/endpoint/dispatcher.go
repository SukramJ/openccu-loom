...
// type-asserting to [matterport.ClusterDataVersion]. Falls back
...
func clusterDataVersion(srv matterport.ClusterServer) uint32 {
	if dv, ok := srv.(matterport.ClusterDataVersion); ok {
...
func clusterDataVersionFor(ep *Endpoint, srv matterport.ClusterServer) uint32 {
...
//     [matterport.ClusterAttributeLister] optional interface.
...
// that implement [matterport.FabricScopedReader] receive MatterReadFiltered
...
func (d *TopologyDispatcher) serversFor(ep *Endpoint, path im.ConcreteAttributePath) []matterport.ClusterServer {
	...
	for _, srv := range all {
		if srv.MatterClusterID() == path.Cluster {
			return []matterport.ClusterServer{srv}
		}
	}
	return nil
}
...
// [matterport.ClusterAttributeLister] interface. When a cluster
// does not implement it, we fall back to [matterport.ClusterServer.MatterReportable]
...
func (d *TopologyDispatcher) attributesFor(srv matterport.ClusterServer, path im.ConcreteAttributePath) []uint32 {
	...
	var extra []uint32
	if lister, ok := srv.(matterport.ClusterAttributeLister); ok {
...
// implements [matterport.FabricScopedReader], MatterReadFiltered is
...
// startReadInteraction → OnlineContext.forFabricFilteredRead.
//
// DataVersion: when srv implements [matterport.ClusterDataVersion]
...
func readOne(ctx context.Context, ep *Endpoint, srv matterport.ClusterServer, path im.ConcreteAttributePath) im.ReadResult {
	...
	if fsr, ok := srv.(matterport.FabricScopedReader); ok {
...
// optional [matterport.ClusterAttributeLister] /
// [matterport.ClusterCommandLister] /
// [matterport.ClusterEventLister] surfaces — clusters that don't
...
// attributes").
func synthesizeGlobalRead(srv matterport.ClusterServer, attrID uint32) (any, bool) {
	switch attrID {
	...
	case cluster.AttrGlobalAcceptedCommandList:
		if lister, ok := srv.(matterport.ClusterCommandLister); ok {
...
func writeOne(ctx context.Context, srv matterport.ClusterServer, path im.ConcreteAttributePath, value im.AttributeValue) im.WriteResult {
	v := value.Value
	if value.IsNull {
		v = nil
	}
...
// the server's [matterport.ClusterAttributeReadPrivilege] optional
...
func (d *TopologyDispatcher) MinReadPrivilege(endpoint uint16, clusterID, attrID uint32) uint8 {
	...
	for _, srv := range ClusterServers(ep) {
		...
		priv, ok := srv.(matterport.ClusterAttributeReadPrivilege)
		if !ok {
			return 1
		}
...
// server's [matterport.ClusterAttributeWritePrivilege] optional
...
func (d *TopologyDispatcher) MinWritePrivilege(endpoint uint16, clusterID, attrID uint32) uint8 {
	...
	for _, srv := range ClusterServers(ep) {
		...
		priv, ok := srv.(matterport.ClusterAttributeWritePrivilege)
		if !ok {
			return 3
		}
...
// looks up the cluster server for (endpoint, clusterID) and consults the
// server's [matterport.ClusterCommandInvokePrivilege] optional
...
func (d *TopologyDispatcher) MinInvokePrivilege(endpoint uint16, clusterID, cmdID uint32) uint8 {
	...
	for _, srv := range ClusterServers(ep) {
		...
		priv, ok := srv.(matterport.ClusterCommandInvokePrivilege)
		if !ok {
			return 3
		}
...
// DataVersion when the server implements [matterport.ClusterDataVersion].
//
...
func (d *TopologyDispatcher) CurrentDataVersion(_ context.Context, endpoint uint16, clusterID uint32) (uint32, bool) {
	...
	for _, srv := range ClusterServers(ep) {
		...
		dv, ok := srv.(matterport.ClusterDataVersion)
		if !ok {
			return 0, false
		}
