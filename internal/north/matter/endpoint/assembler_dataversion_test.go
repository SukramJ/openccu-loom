...
func TestAssemble_BridgedDataVersionSurvivesUnrelatedReassemble(t *testing.T) {
	...
	restore := matterport.InitialDataVersion
	var seed uint32
	matterport.InitialDataVersion = func() uint32 { seed += 100; return seed }
	t.Cleanup(func() { matterport.InitialDataVersion = restore })
	...
	top3, err := a.Assemble(ctx, []endpoint.Snapshot{{CentralName: central, Devices: []*device.Device{devX}}})
	if err != nil {
	...
	top4, err := a.Assemble(ctx, []endpoint.Snapshot{{CentralName: central, Devices: []*device.Device{devX, devZ}}})
	if err != nil {
