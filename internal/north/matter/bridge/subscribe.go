...
// EmitEvent is the bridge-side [matterport.EventEmitter]
// implementation. Cluster servers (or model-side cluster servers
// that implement [matterport.EventReceiver]) call this when
...
func (b *Bridge) EmitEvent(endpoint uint16, cluster, event uint32, data any, priority matterport.EventPriority) {
...
// [matterport.EventEmitter] — the cluster-side hook signature
// the bridge passes to model-side receivers via SetMatterEventEmitter.
func (b *Bridge) MatterEmitEvent(endpoint uint16, cluster, event uint32, data any, priority matterport.EventPriority) {
...
// Measurement source implements [matterport.ChangeNotifier] to
...
func (b *Bridge) wireMeasurementListenersLocked() {
	...
	for _, ep := range b.topology.Endpoints {
		...
		var notifier matterport.ChangeNotifier
		var ok bool
		if ep.Source != nil {
			notifier, ok = ep.Source.(matterport.ChangeNotifier)
		}
		if !ok && ep.Measurement != nil {
			notifier, ok = ep.Measurement.(matterport.ChangeNotifier)
		}
		if !ok {
			continue
		}
		...
		if srv, isSrv := notifier.(matterport.ClusterServer); isSrv {
		...
		for _, srv := range endpointpkg.ClusterServers(ep) {
			...
			subNotifier, isNotifier := srv.(matterport.ChangeNotifier)
			if !isNotifier {
				continue
			}
...
func (b *Bridge) wireMeasurementNotifier(mgr *subscription.Manager, ep *endpointpkg.Endpoint, notifier matterport.ChangeNotifier, pathSet []im.ConcreteAttributePath) {
	epID := ep.ID
...
// implements [matterport.ClusterServer] (e.g. *switchdev.Switch
...
// [matterport.ClusterServer] — the measurement-source path,
...
// endpoints too.
func filterPathsByNotifierCluster(notifier matterport.ChangeNotifier, paths []im.ConcreteAttributePath) []im.ConcreteAttributePath {
	srv, ok := notifier.(matterport.ClusterServer)
	if !ok {
