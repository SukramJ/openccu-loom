...
// ok=false when the row is absent (a database migrated before migration 035)
...
func getNextEndpointIDFromMetadata(ctx context.Context, tx *sql.Tx) (id uint16, ok bool, err error) {
	...
	if err := row.Scan(&v); err != nil {
		...
		return 0, false, fmt.Errorf("matter store: read next_endpoint_id: %w", err)
	}
	if v < firstBridgedEndpointID || v > lastBridgedEndpointID {
		return 0, false, nil
	}
	return uint16(v), true, nil //nolint:gosec // bounded by the range check above; see #20
}
...
func setNextEndpointID(ctx context.Context, tx *sql.Tx, next uint16) error {
	if _, err := tx.ExecContext(
		...
		return fmt.Errorf("matter store: write next_endpoint_id: %w", err)
	}
	return nil
}
