...
func hubGoldenSysvarCases(db *DefaultDiscoveryBuilder) []hubGoldenCase {
	...
	return []hubGoldenCase{
		...
		// The same shape one class further out, and the one that is FIXED
		// rather than merely recorded. This pair is the gap the ADR 0070
		// move-up measurement found: every non-ASCII character in any of
		// the eleven fixture files sat in a `name`, a unit or a
		// TopicSafe-rendered bridge topic — never in an identifier — so the
		// two rules this plane could be moved onto disagreed here with
		// nothing to catch it.
		//
		// [naming.DiscoverySlug] used to transliterate the German set and
		// DROP every other accented Latin character, so "Café" slugged to
		// "caf" and collided with a sysvar actually named "Caf": two
		// objects, one retained config, and Home Assistant kept whichever
		// arrived last. It now delegates to the shared `topic.Slug`, which
		// transliterates it to "cafe". The two topics below are distinct
		// because of that, and they are the visible half of the move —
		// `sysvar/hazard-accent-twin-a`'s topic is one of exactly two
		// (row, field) pairs the unification changed across all eleven
		// fixtures.
		...
		// The second divergence class, also fixed, and the one with a real
		// CCU name behind it — [naming.DiscoverySlug]'s own doc comment
		// cited "Watchdog:_CCU-Jack" as its motivating example for as long
		// as the divergence existed. The colon becomes a separator and
		// meets the literal underscore that follows it; the old rule
		// de-duplicated only the separators it generated and passed the
		// pair through as "watchdog__ccu-jack". The shared `topic.Slug`
		// collapses the run, so the topic below now reads
		// "watchdog_ccu-jack". No fixture carried a literal double
		// underscore in an identifier before this one, so without it the
		// change would have shipped green.
