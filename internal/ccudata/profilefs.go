...
// Open implements [fs.FS]. Only regular files are served: the snapshot's
// profiles/ directory is flat, so a directory handle would have no use
// beyond ReadDir, which is served by [profilesFS.ReadDir].
func (profilesFS) Open(name string) (fs.File, error) {
...
func (f *profileFile) Stat() (fs.FileInfo, error) {
	return profileInfo{name: f.name, size: int64(f.Len())}, nil
}
...
func (e profileDirEntry) Info() (fs.FileInfo, error) { return profileInfo{name: string(e)}, nil }
...
func (i profileInfo) Mode() fs.FileMode  { return 0o444 }
func (i profileInfo) ModTime() time.Time { return time.Time{} }
func (i profileInfo) IsDir() bool        { return false }
