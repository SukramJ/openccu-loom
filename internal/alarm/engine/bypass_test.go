...
// seedAutoBypassArea seeds area "ab" with one instant sensor flagged
// bypass_auto and one plain instant sensor.
func seedAutoBypassArea(h *harness) {
	h.t.Helper()
	h.seedArea("ab", "Anbau", engine.AreaConfig{
...
func TestArm_BypassAutoRecordsExclusionUntilDisarm(t *testing.T) {
	...
	h.wantState("ab", hmenum.AlarmAreaStateArmed)
	...
	h.eng.HandleSensorEvent(h.ctx, "flaky", true)
	h.wantState("ab", hmenum.AlarmAreaStateTriggered)
}
