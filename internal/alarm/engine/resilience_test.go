...
// flakyStateStore fails Get for one area ID.
type flakyStateStore struct {
	*sqlitestore.AlarmStateStore
	failGetID string
}

func (f *flakyStateStore) Get(ctx context.Context, areaID string) (sqlitestore.AlarmStateRow, bool, error) {
	if areaID == f.failGetID {
		return sqlitestore.AlarmStateRow{}, false, errors.New("simulated read failure")
	}
	return f.AlarmStateStore.Get(ctx, areaID)
}
...
func TestStart_PartialFailureCancelsScheduledTimers(t *testing.T) {
	h := newHarness(t)
	h.seedStandardArea() // area "eg"
	h.seedArea("zz", "Zweitbereich", defaultAreaConfig())
	h.start()
