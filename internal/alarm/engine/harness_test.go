...
// openMu), so every harness queued that second behind the same lock — with
// one harness per test that was the bulk of this package's runtime, all of it
// re-deriving a schema that never varies. Deriving it once and copying the
// file gives each test a private database for the price of a file copy.
var migratedSchemaTemplate = sync.OnceValues(func() (string, error) {
	dir, err := os.MkdirTemp("", "alarm-engine-schema-template")
	if err != nil {
		return "", err
	}
