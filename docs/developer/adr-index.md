...
| [0068](../adr/0068-unique-id-stability-per-plane.md) | What `unique_id` promises, and it differs per plane | Immutable on MQTT discovery, where Home Assistant has no migration path at all; changeable on REST/WS with a documented transition and an old key derivable from the payload. A generic `previous_unique_id` was designed and rejected. |
