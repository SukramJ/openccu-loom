...
and [`docs/matter-parity-contract.md`](../matter-parity-contract.md)
...
`docs/parity/by_design.md`. Do not reopen this decision while
implementing.
...
5. Add a `docs/parity/by_design.md` entry recording the NotAvailable
...
- **OTA Provider:** add `cluster/core/parity_matterjs_test.go` cases for
  ...
  `docs/parity/matter/tlv-wire-fixtures.json`; regen the schema snapshot
  `docs/parity/matter/matter-schema-snapshot.json` if the cluster was
...
- **Integration (optional, hermetic):** `tests/integration/` with
  ...
  `docs/matter/chip-tool-test-brief.md` and need explicit user approval
...
- [ ] Every Matter change cites the mirrored matter.js `path:function`
      ...
      `docs/parity/by_design.md` (no audit-tracking tokens in comments —
      `TestDocPurity`).
...
- [`docs/matter-parity-contract.md`](../matter-parity-contract.md) — the
  standing parity guards this work must extend.
- `docs/parity/by_design.md`: L6-PFAD-1/L6-PFAD-3 (shared-ticker /
