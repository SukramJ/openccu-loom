...
> **Status: FIXED (Unreleased).** Bridged endpoints now host their per-cluster
...
> **Status: FIXED (Unreleased).** `tryResume` now adopts the full session
...
- **H1 (CASE) — UpdateNOC persists an unvalidated NOC**, never updates the fabric record or mDNS, never invalidates resumption. `cluster/core/operational_credentials.go:1484` vs `OperationalCredentialsServer.ts:353`, `Fabric.ts:524`. **FIXED (Unreleased):** handleUpdateNOC verifies the chain against the fabric's STORED root (`Fabric.ts:508-538`, chip `FabricTable.cpp:854-882`), rejects a NOC over a key ≠ the pending update-CSR key with InvalidPublicKey (`Fabric.ts:524`, chip `:890`), pins the FabricID (spec §11.18.6.9, chip `fabricIdToValidate`), tracks a NodeID change onto the fabric row (`UpdateFabricNodeID`) with old-instance mDNS withdraw + re-announce + CASE-identity refresh (DeviceAdvertiser.ts:65-76), and the daemon's fabric-updated hook now drops the fabric's resumption records (`FailsafeContext.ts:168-171` replaceFabric).
- **H2 (CASE) — AddNOC has no FabricConflict detection**; the NOC-pubkey-vs-CSR check is missing so `InvalidPublicKey` is unreachable. `cluster/core/operational_credentials.go:1377` vs `FailsafeContext.ts:257`, `Fabric.ts:524`. **FIXED (Unreleased):** the extracted NOC pubkey is compared against the pending CSR key → InvalidPublicKey; a proactive `(FabricID, RootPublicKey)` scan over the installed set rejects a duplicate install with FabricConflict before the store INSERT (mirrors the matter.js globalId check).
...
- NodeLabel/Location writes not persisted across restart. `cluster/core/basic_information.go:500`. **FIXED (Unreleased):** a Matter write to NodeLabel/Location (both "N" quality, §11.1.6) fires `BasicInformation.SetOnPersistentWrite`; the daemon persists them to the new `matter_settings` table (migration 025, `store.SetSetting`) and restores them at boot via `SetNodeLabel`/`SetLocation` before the first Subscribe. matter.js persists them through its behavior state.
