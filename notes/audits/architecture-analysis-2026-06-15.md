...
> **HISTORICAL — a point-in-time reading, not current state.** This analysis
> is retained because ADRs 0030–0038 cite it as the context for their
> decisions; deleting it would orphan the rationale behind that whole ADR
> cluster. It describes the tree as of 2026-06-15 and has not been re-measured
> since — the repo has shipped many releases past it, and most findings below
> were closed by the ADRs that cite them. For what is currently open, read
> [`deep-audit-backlog.md`](./deep-audit-backlog.md); for the release-by-release
...
| `notes/audits/` directory | **absent on disk** | listed in CLAUDE.md repo tree | ⚠️ **drift** |
...
| C5 | CLAUDE.md repo tree lists `notes/audits/` | directory absent on disk (hence the SPEC-referenced `architecture-review-2026-05-05.md` is missing) | confirmed | Low — broken provenance |
...
| Doc: `notes/audits/` exists | absent on disk | **CONFIRMED drift** (C5) |
