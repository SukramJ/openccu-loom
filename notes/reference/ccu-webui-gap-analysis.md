...
`grep -n 'Entscheidung:' notes/reference/ccu-webui-gap-analysis.md`. `abgelehnt`
...
etwa für die `Ref: notes/reference/ccu-webui-gap-analysis.md <ID>`-Zeilen in älteren
...
Auswertung: `grep -n 'Frequenz:' notes/reference/ccu-webui-gap-analysis.md`. Der
...
- **G03** — beim Gerätetausch migriert die CCU Links/Teams/ReGa-Referenzen
  ...
  dokumentierte Lücke (`notes/parity/by_design.md`).
...
- **A3 — Gruppen via HMServer-jpages-Proxy vs. nativer Nachbau.**
  ...
  **Entscheidung:** `entschieden` → Proxy, siehe
  [ADR 0055](../../docs/adr/0055-groups-jpages-proxy.md). Session-Frage geklärt:
...
notes/reference/ccu-webui-gap-analysis.md <ID>`) und in älteren PRs zitiert. **Was ein
