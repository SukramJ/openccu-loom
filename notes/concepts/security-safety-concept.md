...
**Status:** Umgesetzt. Die Domäne ist geschnitten und ausgeliefert —
`internal/security/**` und `internal/model/security/**` existieren, die
MQTT-Ebene ist in [ADR 0059](https://github.com/SukramJ/openccu-loom/blob/main/docs/adr/0059-security-safety-mqtt-plane.md)
festgeschrieben. Dieses Dokument ist damit der Design-Record, kein Plan mehr;
wo Konzept und Code auseinandergehen, gewinnen der Code und `CHANGELOG.md`.
Die ursprünglich hier genannten Ziel-Versionen (Release 0.53.0, API 3.14.0 →
3.15.0) sind überholt und wurden entfernt, weil sie beim Lesen wie offene
Planung wirken.
**Betrifft:** `internal/alarm/**`, `internal/security/**`, `internal/model/security/**`, Nordflächen MQTT/REST/WS/Webhook, SPA
