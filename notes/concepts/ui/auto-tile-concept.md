...
[`notes/plans/roadmap.md`](../../plans/roadmap.md) for what is still open elsewhere in
the SPA.
...
Its two sibling concept docs ([CDP](./cdp-widget-concept.md) and
[Control](./control-widget-concept.md)) describe per-kind and
per-family / per-channel-type code paths. This document describes the
generalisation that sits *under* both and catches whatever the
hand-coded layers don't claim. It also replaced a third, narrower
concept — the per-device-kind `SensorActorTile` — whose composition
logic AutoTile absorbed; `primary.ts` and `classify.ts` survive from it
as internals of the AutoTile composer.
