...
!!! note "Historical migration record"
    ...
    `notes/parity/by_design.md`). The volume snapshot, cluster mapping,
...
- **Architecture-divergence items** (intentional shape mismatches,
  not gaps): `notes/parity/by_design.md` §A4 + §A5.
- **Risk register and prior open items**: see the `notes/parity/by_design.md` divergence catalogue and the Git history.
