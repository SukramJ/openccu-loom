...
| **P2.4** | Persistence decision implemented — see §5.3, the hard rule | A fresh library-only DB passes the store test suite; an existing loom DB migrates with **zero DDL change** | L | D4 ✓ — `EnsureSchema`, greenfield only | P2.1 |
...
| **D4** | Who owns the SQL? | **The concrete `Store` travels; `EnsureSchema` is greenfield-only** | `store/store.go:21` already takes nothing but `*sql.DB` and pulls zero host packages. loom keeps its eleven numbered migrations untouched and never calls `EnsureSchema`; `store.EndpointKey` travels byte-for-byte with a golden test on the rendered key. §5.3 is the hard rule this implements. |
