...
**Ahead of everything else in this phase**, `internal/north/matter/endpoint/topology_golden_test.go` records the assembled topology — endpoint ids, device types, names, unique ids, cluster sets — over a fixed four-device fleet. Every step below has to leave it byte-identical, or explain itself. A changed endpoint id costs every paired controller a re-add (`migrations/007_matter_endpoints.sql`, Down comment), and no structural criterion can see it.
...
| **D19** | Who owns the actuator cluster servers once P3.2 is deferred? | **The library carries them as a labelled conformance reference** | D1(a) put them in the library, which obliged P3.2; deferring P3.2 would have left `cluster/{light,cover,thermo,lock,closure,onoff}` — **1,892 non-test LOC, zero production callers** — as unexplained dead weight. They travel, because what they are actually worth is the matter.js parity coverage they carry: they pin cluster IDs, revisions and mandatory attribute sets against the schema snapshot, which is exactly what a protocol library should assert about itself. The condition is that they say so. `windowcovering_server.go:147-151` already does — *"they return Success without forwarding to a CCU backend"* — and each of the six needs the same statement plus a guard that fails if one ever acquires a production caller without P3.2 being taken up again. The real actuator servers stay in `internal/model/custom/*`, untouched. |
