...
* **D1 is left in tension and is re-opened as D19.** D1(a) obliges P3.2 — the library owning the cluster servers — and deferring P3.2 leaves 1,892 LOC of stand-ins in the library with no production caller.
...
| **R14** | **The three generic servers are conformance stand-ins, not spare implementations** | `cluster/light` (246), `cluster/cover` (237), `cluster/thermo` (583) have **0** non-test importers (reproduced; `cluster/lock` returns 2 with the same pattern, so the measurement bites). `windowcovering_server.go:147-151` says in source that its commands return Success *without forwarding to a CCU backend* | Medium → **High if P3.2 is read as a move** | P3.2a raises them to production capability *before* P3.2c switches loom over. Reading P3.2 as "the servers already exist, just re-point the callers" would swap field-proven code for code that has never driven a device |
...
| **D19** | **OPEN** — who owns the actuator cluster servers once P3.2 is deferred? | *unanswered* | D1(a) put them in the library, which obliges P3.2. Deferring P3.2 leaves `cluster/{light,cover,thermo,lock,closure,onoff}` — **1,892 non-test LOC**, all with zero production callers and, per `windowcovering_server.go:147-151`, returning Success without forwarding — inside go-fabric with no user. Three coherent resolutions: carry them as an honestly-labelled conformance reference that keeps the matter.js parity tests alive; delete them and let go-fabric be protocol, core and sensing only; or revise D1 to (b), which makes them consistently host-side at the price of publishing `wire` and `im` forever. Gates the shape of go-fabric, not its existence. |
...
**Open decisions remaining: one — D19**: who owns the actuator cluster servers now that P3.2 is deferred? It gates the shape of go-fabric, not whether it is extracted. D18 settled the goal question on 2026-09-05, D17 the module name. A question that arises during the work becomes a new dated row here.
